...
public class NetworkTransformInterpolation : MonoBehaviour
{
	...
	{
		bufferedStates = null;
		bufferedStates = new NetworkTransform[20];
	}
	...
	public void ReceivedTransform(NetworkTransform ntransform)
	{
		...
		{
			if (bufferedStates[i] != null && bufferedStates[i + 1] != null && bufferedStates[i].TimeStamp < bufferedStates[i + 1].TimeStamp)
			{
	...
	private void Update()
	{
		if (!running || statesCount == 0 || SmartFoxConnection.Connection == null || SmartFoxConnection.Connection.TimeManager == null)
		{
			return;
		}
		UpdateValues();
		double networkTime = SmartFoxConnection.Connection.TimeManager.NetworkTime;
		...
		if (mode == InterpolationMode.INTERPOLATION && bufferedStates[0].TimeStamp > num)
		{
			...
			{
				if (bufferedStates[i] != null && (bufferedStates[i].TimeStamp <= num || i == statesCount - 1))
				{
	...
	private void UpdateValues()
	{
		...
		{
			double averagePing = SmartFoxConnection.Connection.TimeManager.AveragePing;
			if (averagePing < 50.0)
			{
