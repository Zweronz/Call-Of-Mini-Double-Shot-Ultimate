...
public class DDSMap5TriggerScript : DDSTriggerScript
{
	...
	private float m_WaveRefreshTimeInterval = 1f;
			...
			if (num2 < emenySpawnLimit)
			{
				...
				if (subWaveIndex < curWave.subWaveTimes.Length)
				{
					...
					if (subWaveIndex < curWave.subWaveTimes.Length)
					{
						...
						{
							int num4 = int.Parse(key);
							int num5 = int.Parse(hashtable[key].ToString());
