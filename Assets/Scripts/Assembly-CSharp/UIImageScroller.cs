...
public class UIImageScroller : UIPanel, UIHandler
{
	...
	{
		eventRect = AutoUIResolution.ToShiftToRight(eventRect, 2);
		clipRect = AutoUIResolution.ToShiftToRight(clipRect, 2);
	...
	{
		m_CenterFrameImage.SetTexture(material, rect, new Vector2(rect.width, rect.height));
		if (m_Dir == ScrollerDir.Vertical)
		{
			float left = m_ClipRect.x + 0.5f * m_ClipRect.width - 0.5f * rect.width;
			float top = m_ClipRect.y + 0.5f * m_ClipRect.height - 0.5f * rect.height;
			m_CenterFrameImage.Rect = new Rect(left, top, rect.width, rect.height);
		}
	}
	...
	public void SetMaskImage(Material material, Rect texRect, Vector2 size)
	{
		for (int i = 0; i < m_MaskList.Count; i++)
		{
			UIImage uIImage = m_MaskList[i];
			uIImage.SetTexture(material, texRect, size);
		}
	}
	...
	public void HandleEvent(UIControl control, int command, float wparam, float lparam)
	{
		...
		case 2:
		{
			float x = Mathf.Clamp(m_LastMove.x, -30f, 30f);
		...
		case 0:
			if (m_EnableLongPress)
			{
				...
				float left = (int)(m_ClipRect.x + 0.5f * (m_ClipRect.width - (float)m_IconWidth));
				float centerPos = m_CenterPos;
				if (new Rect(left, centerPos, m_IconWidth, m_IconHeight).Contains(new Vector2(wparam, lparam)) && m_lastBeginPressTime == -1f)
				{
	...
	public override void Draw()
	{
		...
		{
			Debug.Log("ERROR - 222222222222222");
		}
		...
		for (int k = 0; k < m_Controls.Count - 1; k++)
		{
			...
			else
			{
				...
				if (m_Scalable)
				{
					...
					if (m_overlayInfoList.Count > 0)
					{
						m_overlayImageList[num14].SetTextureSize(new Vector2(m_overlayInfoList[0].m_TexRect.width * num18, m_overlayInfoList[0].m_TexRect.height * num18));
					}
				}
