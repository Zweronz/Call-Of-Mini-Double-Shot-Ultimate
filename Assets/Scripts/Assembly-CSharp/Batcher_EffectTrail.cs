...
public class Batcher_EffectTrail : MonoBehaviour
{
	...
	public bool m_bCollectPoints;

	public float m_lastTime = 0.5f;

	private float m_time;

	private GameObject m_TrailObj;
	...
	private void Start()
	{
		m_TrailObj = new GameObject(base.gameObject.transform.root.gameObject.name + "_Trail");
		MeshFilter meshFilter = m_TrailObj.AddComponent(typeof(MeshFilter)) as MeshFilter;
		MeshRenderer meshRenderer = m_TrailObj.AddComponent(typeof(MeshRenderer)) as MeshRenderer;
		meshRenderer.material = m_TrailMaterial;
		meshFilter.mesh = new Mesh();
		mesh = meshFilter.mesh;
		base.gameObject.transform.root.gameObject.GetComponent<Animation>()["Attack01"].wrapMode = WrapMode.Loop;
		base.gameObject.transform.root.gameObject.GetComponent<Animation>().Play("Attack01");
	}
	...
	private void UpdateMesh()
	{
		if (m_bCollectPoints)
		{
			...
			tronTrailSection.m_vertexUp = position + forward * m_height;
			tronTrailSection.m_vertexDown = position;
